...
double sumMatrix(std::vector<std::vector<double>> matrix){
    ...
    for (int col = 0; col < matrix.size(); col++){
        for (int row = 0; row < matrix.size(); row++){
            double value = matrix.at(col).at(row); 
            //Wanna go through columns first to not access the RAM each time
            sum += value;            
        }
    }
    return sum;
...
void optimizationTask() {
    ...
    matrix.reserve(matrixSize); //reserve
    
    for (int i = 0; i < matrixSize; i++){
        std::vector<double> column;
        column.reserve(matrixSize); //reserve
